...
impl VL53L0x {
  ...
  pub fn new() -> Result<VL53L0x, Error> {
    VL53L0x::with_address(DEFAULT_ADDRESS)
  }

  /// Creates new driver with given address.
  pub fn with_address(address: u16) -> Result<VL53L0x, Error> {
  ...
  pub fn set_address(&mut self, new_address: u16) -> Result<(), Error> {
    match self.com.set_slave_address(new_address) {
      Ok(_) => Ok(()),
      Err(e) => Err(Error::BusError(e)),
    }
  }

  fn power_on(&mut self) -> Result<(), rppal::i2c::Error> {
    // TODO use pin to poweron
    // XXX: not-needed?
    Ok(())
  }

  fn read_register(&mut self, reg: Register) -> Result<u8, rppal::i2c::Error> {
    let mut data: [u8; 1] = [0];
    // FIXME:
    //  * device address is not a const
    //  * register address is u16
    self.com.block_read(reg as u8, &mut data)?;
    Ok(data[0])
  }

  fn read_byte(&mut self, reg: u8) -> Result<u8, rppal::i2c::Error> {
    let mut data: [u8; 1] = [0];
    // FIXME:
    //  * device address is not a const
    //  * register address is u16
    self.com.block_read(reg, &mut data)?;
    Ok(data[0])
  }

  fn read_6bytes(&mut self, reg: Register) -> Result<[u8; 6], rppal::i2c::Error> {
    let mut ret = [0u8; 6];
  ...
  fn read_registers(&mut self, reg: Register, buffer: &mut [u8]) -> Result<(), rppal::i2c::Error> {
    // const I2C_AUTO_INCREMENT: u8 = 1 << 7;
    // const I2C_AUTO_INCREMENT: u8 = 0;
    self.com.block_read(reg as u8, buffer)?;

    Ok(())
  }

  fn read_16bit(&mut self, reg: Register) -> Result<u16, rppal::i2c::Error> {
    let mut buffer: [u8; 2] = [0, 0];
    self.read_registers(reg, &mut buffer)?;
    Ok((cast::u16(buffer[0]) << 8) + cast::u16(buffer[1]))
  }

  fn write_byte(&mut self, reg: u8, byte: u8) -> Result<(), rppal::i2c::Error> {
    self.com.block_write(reg as u8, &[byte])
  }

  fn write_register(&mut self, reg: Register, byte: u8) -> Result<(), rppal::i2c::Error> {
    self.com.block_write(reg as u8, &[byte])
  }

  fn write_6bytes(&mut self, reg: Register, bytes: [u8; 6]) -> Result<(), rppal::i2c::Error> {
    self.com.block_write(
      reg as u8,
      &[bytes[0], bytes[1], bytes[2], bytes[3], bytes[4], bytes[5]],
    )
  }

  fn write_16bit(&mut self, reg: Register, word: u16) -> Result<(), rppal::i2c::Error> {
    // let mut buffer = [0];
    // let msb = (word >> 8) as u8;
    // let lsb = (word & 0xFF) as u8;
    // self.com.write_read(&[reg as u8, msb, lsb], &mut buffer);
    self.com.block_write(reg as u8, &word.to_le_bytes())
  }

  fn write_32bit(&mut self, reg: Register, word: u32) -> Result<(), rppal::i2c::Error> {
    // let mut buffer = [0];
    ...
    // self
    //   .com
    //   .write_read(&[reg as u8, v1, v2, v3, v4], &mut buffer)
    self.com.block_write(reg as u8, &word.to_le_bytes())
  }

  fn set_signal_rate_limit(&mut self, limit: f32) -> Result<bool, rppal::i2c::Error> {
  ...
  fn get_spad_info(&mut self) -> Result<(u8, u8), Error> {
  ...
  /// startContinuous
  pub fn start_continuous(&mut self, period_millis: u32) -> Result<(), rppal::i2c::Error> {
    self.write_byte(0x80, 0x01)?;
    self.write_byte(0xFF, 0x01)?;
    self.write_byte(0x00, 0x00)?;
  ...
  /// stopContinuous()
  pub fn stop_continuous(&mut self) -> Result<(), rppal::i2c::Error> {
      ...
      Ok(r) => {
        if (r & 0x07) == 0 {
          ...
          match (range_err, write_err) {
            (Ok(res), Ok(_)) => Ok(res),
            (Err(e), _) => Err(Error::BusError(e)),
            (_, Err(e)) => Err(Error::BusError(e)),
          }
        }
      }
      Err(e) => Err(Error::BusError(e)),
  ...
  pub fn read_range_continuous_millimeters_blocking(&mut self) -> Result<u16, Error> {
    let mut c = 0;
  ...
  pub fn read_range_single_millimeters_blocking(&mut self) -> Result<u16, Error> {
    self.write_byte(0x80, 0x01)?;
    self.write_byte(0xFF, 0x01)?;
    self.write_byte(0x00, 0x00)?;
    let sv = self.stop_variable;
  ...
  /// Returns who am i
  pub fn who_am_i(&mut self) -> Result<u8, rppal::i2c::Error> {
    self.read_register(Register::WHO_AM_I)
  }

  fn get_vcsel_pulse_period(&mut self, ty: VcselPeriodType) -> Result<u8, rppal::i2c::Error> {
    match ty {
  ...
  fn get_sequence_step_enables(&mut self) -> Result<SeqStepEnables, rppal::i2c::Error> {
  ...
  ) -> Result<SeqStepTimeouts, rppal::i2c::Error> {
    let pre_range_mclks =
  ...
  fn get_measurement_timing_budget(&mut self) -> Result<u32, rppal::i2c::Error> {
